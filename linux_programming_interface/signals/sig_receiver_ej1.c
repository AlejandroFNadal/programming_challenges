...
/* sig_receiver.c
  ...
  Resolution of Exercise 1: Modify sig_receiver.c to use sigaction() rather than
  signal() to establish the signal handler. Verify that the program continues to
  work as before.
*/
#define _GNU_SOURCE
...
static struct sigaction struct_array_sigactions[NSIG];
...
int main(int argc, char *argv[]) {
  ...
  for (n = 1; n < NSIG; n++) {
    ...
    struct_array_sigactions[n].sa_handler = &handler;
    struct_array_sigactions[n].sa_flags =
        SA_NODEFER; // I think this is necessary if lots of signals are sent at
                    // the same time
    sigaction(n, &struct_array_sigactions[n], NULL);
  }
