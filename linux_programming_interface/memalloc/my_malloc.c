...
int add_used_block(size_t size){
  ...
  area *block = (area*)(heap_start + FIRST_BLOCK_OFFSET);
  area *smallest_block = NULL;
  while (block != NULL){
    assert(block->marker == BLOCK_MARKER);
    if((block->length + sizeof(block)) >= size && block->in_use == false) {
  ...
  if(smallest_block != NULL){
    ...
    if(remaining_size > 0){
      area *new_block = (area*) smallest_block + sizeof(smallest_block) +smallest_block->length;
      ...
      new_block->length = smallest_block->size - size
    }
    smallest_block->length = size;
  }
  malloc_header->my_simple_lock = false; 
  return smallest_block + sizeof(*free_area);
}
